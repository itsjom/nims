...
<body class="antialiased text-slate-800 min-h-screen py-10 px-4 flex justify-center items-start">
    ...
    <div class="max-w-xl w-full bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 p-8 relative">
        ...
        <div class="absolute -top-24 -right-24 w-48 h-48 bg-green-100 rounded-full blur-3xl opacity-50 pointer-events-none"></div>
        <div class="absolute -bottom-24 -left-24 w-48 h-48 bg-slate-100 rounded-full blur-3xl opacity-50 pointer-events-none"></div>

        <div class="absolute top-4 right-4 z-20">
            <a href="{{ route('dashboard') }}" class="p-2 bg-slate-50 hover:bg-rose-50 text-slate-400 hover:text-rose-500 rounded-full transition-colors flex items-center justify-center">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </a>
        </div>

        <div class="text-center mb-8 relative z-10">
            <img src="{{ asset('images/ncf-logo.png') }}" alt="NCF Logo" class="w-16 h-16 mx-auto object-contain drop-shadow-sm mb-4 bg-white rounded-full p-1 shadow-sm">
        ...
        @if($errors->any())
            <div class="bg-rose-50 text-rose-700 p-4 rounded-xl mb-6 font-medium border border-rose-200 shadow-sm relative z-10">
        ...
        <form action="{{ route('return.store') }}" method="POST" class="space-y-5 relative z-10">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Borrow Record <span class="text-rose-500">*</span></label>
                    <select name="borrow_id" required id="borrow_id_select"
                        ...
                        @foreach($borrowLogs as $log)
                            <option value="{{ $log->borrow_id }}" data-equipment="{{ $log->equipment }}">
                                {{ $log->formatted_id }} - {{ $log->equipment }} (Qty: {{ $log->quantity }}, by {{ $log->student_name }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="bg-green-50/50 p-4 rounded-xl border border-green-100/50 space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Equipment Name <span class="text-rose-500">*</span></label>
                    ...
                    <p class="text-xs text-slate-500 mt-1.5 ml-1 tracking-wide">Must match the borrowed equipment to properly restore stock.</p>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Quantity Returned <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Condition Details <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Received By <span class="text-rose-500">*</span></label>
            ...
            <div class="text-center mt-4">
                <a href="{{ route('dashboard') }}" class="text-xs font-medium text-slate-400 hover:text-slate-600 transition-colors">Return to Dashboard</a>
            </div>
        </form>
        ...
        document.getElementById('borrow_id_select').addEventListener('change', function() {
            ...
            // Re-enable temporarily to change value if it might fail
            let found = false;
            ...
            if(!found) {
                // Pre-select disabled or alert if we can't find it exactly?
