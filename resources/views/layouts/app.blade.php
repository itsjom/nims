    ...
    <div class="flex h-screen overflow-hidden bg-slate-50">
        <aside class="w-72 bg-white border-r border-slate-200 flex flex-col transition-all duration-300 z-20 shadow-sm relative">
            <div class="h-20 flex items-center px-8 border-b border-slate-100">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-indigo-200">N</div>
                <span class="ml-4 text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-slate-800">NIMS</span>
            </div>
            <nav class="flex-1 overflow-y-auto sidebar-scroll py-6 px-4 space-y-1.5">
                ...
                <a href="/" class="flex items-center px-4 py-3 {{ request()->is('/') ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:text-indigo-600' }} rounded-xl font-medium transition-all duration-200 group relative">
                    @if(request()->is('/'))
                    <div class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-indigo-600 rounded-r-full"></div>
                    @endif
                    <svg class="w-5 h-5 mr-3 {{ request()->is('/') ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                    Dashboard
                </a>
                <div class="space-y-1">
                    @php 
                    ...
                    <button id="inventory-btn" class="w-full flex items-center justify-between px-4 py-3 {{ $isInventory ? 'text-indigo-700' : 'text-slate-600 hover:text-indigo-600' }} rounded-xl font-medium transition-all duration-300 group">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3 {{ $isInventory ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-500' }} transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                            Inventory
                        </div>
                        <svg id="inventory-icon" class="w-4 h-4 {{ $isInventory ? 'text-indigo-600 rotate-180' : 'text-slate-400' }} transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                ...
                <div class="space-y-1 mt-2">
                    ...
                    @endphp
                    <button id="logs-btn" class="w-full flex items-center justify-between px-4 py-3 {{ $isLogs ? 'text-indigo-700' : 'text-slate-600 hover:text-indigo-600' }} rounded-xl font-medium transition-all duration-300 group">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3 {{ $isLogs ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-500' }} transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4h4m-4-8a8 8 0 110 16 8 8 0 010-16z"></path></svg>
                            Logs
                        </div>
                        <svg id="logs-icon" class="w-4 h-4 {{ $isLogs ? 'text-indigo-600 rotate-180' : 'text-slate-400' }} transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div id="logs-submenu" class="pl-11 pr-4 space-y-1 overflow-hidden transition-all duration-300 {{ $isLogs ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0' }}">
                        <div class="py-1">
                            <a href="{{ route('logs.borrowing') }}" class="block px-3 py-2 text-sm font-medium {{ request()->routeIs('logs.borrowing') ? 'text-indigo-700 bg-indigo-50' : 'text-slate-500 hover:text-indigo-600' }} rounded-lg transition-colors">Borrowing Log</a>
                            <a href="{{ route('logs.returned') }}" class="block px-3 py-2 text-sm font-medium {{ request()->routeIs('logs.returned') ? 'text-indigo-700 bg-indigo-50' : 'text-slate-500 hover:text-indigo-600' }} rounded-lg transition-colors">Return Log</a>
                        </div>
                    </div>
                </div>
                
                <!-- CI Monitoring Menu -->
                <a href="{{ route('ci-monitoring.index') }}" class="flex items-center px-4 py-3 {{ request()->routeIs('ci-monitoring.*') ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:text-indigo-600' }} rounded-xl font-medium transition-all duration-200 group relative mt-2">
                    @if(request()->routeIs('ci-monitoring.*'))
                    <div class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-indigo-600 rounded-r-full"></div>
                    @endif
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('ci-monitoring.*') ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    CI Monitoring
                </a>
            </nav>
            <div class="p-6 border-t border-slate-100">
                <div class="flex items-center gap-3 w-full cursor-pointer group">
                    <img src="https://ui-avatars.com/api/?name=Admin+User&background=6366f1&color=fff" alt="User Avatar" class="w-10 h-10 rounded-full shadow-sm group-hover:ring-2 ring-indigo-200 transition-all">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-800 truncate">Admin User</p>
                        <p class="text-xs text-slate-500 truncate">admin@nims.com</p>
                    </div>
                </div>
            </div>
        </aside>
