...
@section('content')
            ...
            <tbody id="dataTable" class="divide-y divide-slate-100">
                @forelse($items as $item)
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span
                                onclick="openDetailsModalNcon('{{ $item->item_code }}', '{{ addslashes($item->item_name) }}', '{{ $item->unit }}', '{{ $item->ideal_stocks }}', '{{ $item->total_stock }}', '{{ $item->supply_on_hand }}', '{{ $item->location }}', '{{ $item->item_condition }}')"
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button
                                    onclick="openEditModalNcon({{ $item->id }}, '{{ addslashes($item->item_name) }}', '{{ $item->unit }}', '{{ $item->ideal_stocks }}', '{{ $item->total_stock }}', '{{ $item->supply_on_hand }}', '{{ $item->location }}', '{{ $item->item_condition }}')"
                                ...
                                <form action="{{ route('csr-ncon-t1.destroy', $item->id) }}" method="POST" class="inline-block"
    ...
    <div id="detailsModalNcon"
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden my-8">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <button onclick="document.getElementById('detailsModalNcon').classList.add('hidden')"
    ...
    <div id="editModal"
        ...
        <div class="relative w-full max-w-xl bg-white rounded-2xl shadow-xl overflow-hidden my-8">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800">Edit CSR-NCON-T1 Item</h3>
    ...
    <div id="addModal"
        ...
        <div class="relative w-full max-w-xl bg-white rounded-2xl shadow-xl overflow-hidden my-8">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800">Add New CSR-NCON-T1 Item</h3>
            ...
            <form action="{{ route('csr-ncon-t1.store') }}" method="POST" enctype="multipart/form-data" class="p-6"
                id="addFormNcon">
                @csrf
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                    ...
                    <div>
                        ...
                        <input type="text" id="conditionOtherNcon" placeholder="Specify condition"
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="file" name="image" accept="image/*"
                            class="seq-input-ncon w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-green-500 transition-colors file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-medium file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const form = document.getElementById('addFormNcon');
        if (!form) return;

        const elements = Array.from(form.querySelectorAll('.seq-input-ncon'));
    ...
    function openEditModalNcon(id, name, unit, ideal, total, on_hand, location, condition) {
        const modal = document.getElementById('editModal');
        const form = document.getElementById('editForm');

        // Setup action route
        form.action = `/csr-ncon-t1/${id}`;
    ...
    function openDetailsModalNcon(code, name, unit, ideal, total, on_hand, location, condition) {
        ...
        document.getElementById('detailsModalNcon').classList.remove('hidden');
    }
</script>