...
<div class="flex justify-between items-center mb-6">
    <div class="relative w-80">
        <input type="text" placeholder="Search return records..." class="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all bg-white shadow-sm">
    ...
    <table class="w-full min-w-max text-left text-sm whitespace-nowrap">
        ...
        <tbody class="divide-y divide-slate-100">
            ...
            <tr class="hover:bg-slate-50/80 transition-colors group">
                <td class="px-5 py-4">
                    <span class="font-mono text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-200">{{ $log->formatted_id }}</span>
                </td>
                ...
                <td class="px-5 py-4 font-semibold text-indigo-700">{{ $log->equipment }}</td>
                <td class="px-5 py-4 text-center font-bold text-slate-700">{{ $log->quantity_returned }}</td>
                <td class="px-5 py-4">
                    <span class="px-2.5 py-1 rounded-full text-xs font-medium 
                        {{ strtolower($log->condition) === 'good' ? 'bg-emerald-100 text-emerald-700' : '' }}
